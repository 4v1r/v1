...
function startHumanTurn() {
  ...
  var col = $('.click-columns div:hover').connect();
  if(col !== -1) {
...
function createLitCell(col, row) {
  $('<div>')
  .css({
    'left': connectToPixels(col),
    'bottom': connectToPixels(row)
  })
  .appendTo('.lit-cells');
}
...
function moveCursorChip(col, callback) {
  $('.chips .cursor').css('left', connectToPixels(col));
  $('.lit-columns .lit').removeClass('lit');
  $('.lit-columns div').eq(col).addClass('lit');
...
function connectToPixels(connect) {
  return (connect * 61 + 1) + 'px';
}